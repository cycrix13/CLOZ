...
OnClickListener {
	...
	@ViewById(id = R.id.pager)			private VerticalPager mPager;
...
//	private int mSelecting = 0;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		loadImage(selecting);
		updateLookInfo(selecting);
		
		mPager.setListener(new VerticalPager.Listener() {
			@Override
			public void onPageChange(VerticalPager pager) {
				updateLookInfo(pager.getCurrentPageIndex());
			}
		});
		
		mPager.setAdapter(new LookPagerAdapter(), lookList.size() - 1);
	...
	class LookPagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, int pageIndex) {
			...
			ZoomPager pager = new ZoomPager(MainActivity2.this);
...
//					prepareForLoadImage(false);
			...
			if (mPathList.size() > 0) {
				ZoomView zoomView = new ZoomView(MainActivity2.this, mPathList.size() > 1);
				...
				ImageView img = new NoImageView(MainActivity2.this, listener);
...
//		}
	...
	private void loadImage(int selecting) {
		
		mPager.setAdapter(new LookPagerAdapter(), selecting);
	}
	
	private void updateLookInfo(int selecting) {
		
		int visibility = selecting >= 0 ? View.VISIBLE : View.INVISIBLE;

		mTxtEdit.setVisibility(visibility);
		mLayoutSeperate.setVisibility(visibility);
		if (selecting >= 0 && !mEditing)
...
//		}
		...
		if (mPager.getCurrentPage() instanceof ViewPager) {
			mIndicator.setVisibility(View.VISIBLE);
			ViewPager pager = (ViewPager) mPager.getCurrentPage();
			LookAdapter adapter = (LookAdapter) pager.getAdapter();
			mIndicator.setPager(pager);
			mIndicator.setOnPageChangeListener(adapter);
		} else {
		...
		if (selecting >= 0) {
			String dateJson = lookList.get(selecting).date;
			try {
		...
		if (mResultMode) {
			String tagJson = lookList.get(selecting).tags;
			try {
			...
			String contactJson = lookList.get(selecting).contacts;
			try {
	...
	private void deleteTag(final String text, final boolean isTag) {
		...
		.setPositiveButton(R.string.text_ok,
			...
			public void onClick(DialogInterface dialog,
					int which) {
				Looks item = lookList.get(mPager.getCurrentPageIndex());
				String json = null;
				if (isTag)
					json = item.tags;
				else
				...
				updateLookInfo(mPager.getCurrentPageIndex());
			}
		}).setNegativeButton(R.string.text_cancel, null)
		.create().show();
	}
							...
							public void onComplete(HashMap<String, Object> result) {
								...
								try {
									...
									for (ImageItem imageItem : imageList) {
										...
										String randomPath = generateRandomImageFileName(MainActivity2.this);
										File dstFile = getFileStreamPath(randomPath);
										try {
											copy(srcFile, dstFile);
										} catch (IOException e) {
	...
	private void refreshCurrentPage() {
		...
		if (mPager.getCurrentPageIndex() < 0)
			return;
		
		String fileNames = lookList.get(mPager.getCurrentPageIndex()).fileName;
		ViewPager pager = (ViewPager) mPager.getCurrentPage();
	...
	private void onAddTagClick(View v) {
		...
		.setItems(R.array.look_msg_add_tag,
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				case 0:
				{
					Looks look = lookList.get(mPager.getCurrentPageIndex());
					ArrayList<String> contactList = new ArrayList<String>();
					JSONArray jContact;
					try {
					...
					ContactListActivity.newInstance(MainActivity2.this,
				...
				case 1:
					TagActivity.newInstance(MainActivity2.this,
	...
	private void onDeleteClick(View v) {
		...
		int seleting = mPager.getCurrentPageIndex();
		if (lookList.size() == 0 || seleting == -1) {
			onEditClick(null);
			return;
		}
		...
		.setPositiveButton(R.string.text_ok,
			...
			public void onClick(DialogInterface dialog,
					int which) {
				int seleting = mPager.getCurrentPageIndex();
				Looks item = lookList.get(seleting);
				...
				if (lookList.size() == 0) {
					...
					seleting = Math.min(seleting,lookList.size() - 1);
				}
	...
	void onShareCallback(Bitmap bm, int socialType, boolean saveImage) {
		...
		case 0:	// Fb
			...
			adapter.authorize(MainActivity2.this, Provider.FACEBOOK);
			break;
		case 1:	// Twit
